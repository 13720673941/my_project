...
class Quit_Group(unittest.TestCase):

    """ 【成员退出圈子校验】 """

    # 实例化操作类
    readExcel = Read_Excel("quitGroup")
    ...
    def setUpClass(cls):
        ...
        cls.assert_mode = Assert(cls.driver,"quitGroup")
        # 登录群主网点
        cls.login.login_main("T西安乐家家政")
        # 进入圈子列表页面
        cls.edit_group.enter_join_group_list_page()
        cls.base.sleep(2)

    @unittest.skipUnless(readExcel.get_isRun_text("quit_group_001"),"-跳过不执行该用例")
    def test_quit_group001(self):
        """退出圈子-订单未结算的成员不能退出校验"""

        # 获取测试数据
        data = self.readExcel.get_dict_data("quit_group_001")
        # 打印测试用例名称
        ...
        self.base.sleep(2)
        # 点击退出按钮
        self.edit_group.click_quit_group_btn()
        # 断言
        self.assert_mode.assert_equal(data,self.login.get_system_msg())

    @unittest.skipUnless(readExcel.get_isRun_text("quit_group_002"),"-跳过不执行该用例")
    def test_quit_group002(self):
        """退出圈子-成员成功退出圈子校验"""

        # 获取测试数据
        data = self.readExcel.get_dict_data("quit_group_002")
        # 打印测试用例名称
        self.base.print_case_name(data)
        # 进入全部工单页面
        ...
        self.base.sleep(2)
        # 进入圈子列表页面
        self.edit_group.enter_join_group_list_page()
        # 点击圈子进入详情页
        self.edit_group.open_group_details(data["圈子名称"])
        self.base.sleep(2)
        # 点击退出按钮
...
if __name__ == '__main__':
    ...
    suits = unittest.TestLoader().loadTestsFromTestCase(Quit_Group)
