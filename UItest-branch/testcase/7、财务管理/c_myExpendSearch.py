...
"""
我的收入页面搜索功能测试用例：
1、按支出类型搜索校验 2、按服务类型搜索校验 3、按服务师傅名称搜索校验 4、按工单编号搜索校验
"""
# 获取测试数据
test_data = get_test_data()["MyExpendPage"]

class My_Expend_Search(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.my_expend = FinanceManagePage(cls.driver)
        # 开始执行测试用例
        mytest.start_test()
        # 获取测试账号信息
        ...
        # 进入我的收入页面
        cls.my_expend.enter_my_expend_page()
    ...
    def test_my_expend_search002(self):
        ...
        self.base_page.print_case_name(data["CaseName"])
        # 选择服务类型
        self.my_expend.select_server_type(server_type=data["ServerType"])
        # 点击搜索
        self.my_expend.click_search_btn()
        self.base_page.sleep(1)
        # 获取搜索后第一行所有信息
    ...
    def test_my_expend_search003(self):
        """按服务师傅名称搜索校验"""
        ...
        self.base_page.print_case_name(data["CaseName"])
        # 输入师傅名称
        self.my_expend.input_server_master_name(master_name=data["MasterName"])
        # 点击搜索
        self.my_expend.click_search_btn()
        self.base_page.sleep(1)
        # 获取搜索后第一行所有信息
    ...
    def test_my_expend_search004(self):
        """按工单编号搜索校验"""
        ...
        # 输入工单号
        self.my_expend.input_order_number(order_number)
        # 点击搜索
        self.my_expend.click_search_btn()
        self.base_page.sleep(1)
        # 获取搜索后第一行所有信息
    ...
    def test_my_expend_search005(self):
        """按结算日期支出结算搜索校验"""
        ...
        # 输入开始时间
        self.my_expend.input_settle_start_time(start_time=now_date)
        # 输入结束时间
        self.my_expend.input_settle_end_time(end_time=now_date)
        # 点击查询
        self.my_expend.click_find_btn()
        self.base_page.sleep(2)
        # 获取第一行查询的日期,获取的日期包含时间需要进行分割
        get_date = self.my_expend.get_expend_date().split(" ")[0]
        # 断言
...
if __name__ == '__main__':
    ...
    suits = unittest.TestLoader().loadTestsFromTestCase(My_Expend_Search)
