...
class Server_Set(unittest.TestCase):
    ...
    #     # 经销商下单程序下单
    #     self.create_order.create_order_main(user, phe, address, collage, order_type,
    #                                         server, brands, kinds)
    #     # 获取新建工单单号
    #     self.order_number = self.base_page.get_order_number()
    #     # 进入订单派单页面
    ...
    #     self.server_branch.clear_server_type()
    #     self.base_page.sleep(1)
    #     # 把服务类型数据放入列表中，选择类型只接受list类型
    #     server_list = data["ServerType"].split(",")
    #     # 选择其他服务类型
    #     self.server_branch.select_server_type_of_other(server_name=server_list)
    #     # 清空授权品类
    #     self.server_branch.clear_kinds_type()
    #     self.base_page.sleep(1)
    #     # 获取品类列表
    #     kind_list = data["Kinds"].split(",")
    #     # 选择品类
    #     self.server_branch.select_kinds_type_of_other(kinds_name=kind_list)
    #     # 点击保存
    #     self.server_branch.click_save_set_server()
    #     self.base_page.sleep(2)
    #     # 刷新页面
    ...
    #     self.server_branch.click_server_branch_table()
    #     # 搜索网点
    #     self.server_branch.input_search_branch_keyword(data["SetBranch"])
    #     # 点击搜索
    #     self.server_branch.click_search_branch_btn()
    #     self.base_page.sleep(1)
    #     # 获取第一行所有信息
    #     first_branch_info = self.server_branch.get_first_branch_info()
    #     # 断言
    #     self.assert_mode.assert_more_str_in(data,first_branch_info)
    ...
    def test_server_set011(self):
        ...
        # 选择订单
        self.base_page.select_new_order(OrderNumber="636984609854373061")
        # 点击派单
        self.please_order.click_pleaseOrder_btn()
        # 派单到服务商
        self.please_order.please_to_branch()
        ...
        self.please_order.click_search_branch()
        # 判断服务商搜索不出来
        branch_is_displayed = self.please_order.search_branch_is_display()
        # 断言
        self.assert_mode.assert_el_not_in_page(branch_is_displayed)
