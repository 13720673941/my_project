...
class Send_Email(object):
    ...
    def AttNewReport(self):
        """获取最新的测试报告路径"""
        ReportDirs = os.listdir(reportSavePath)
        # 跳过为空的文件夹
        if len(ReportDirs) != 0:
            # 把获取的报告文件列表文件按照创建顺序排列
            NewReportList = sorted(ReportDirs,key=lambda x:os.path.getctime(os.path.join(reportSavePath,x)))
            # 获取最后一个报告就是最新的报告
            NewReport = reportSavePath + NewReportList[-1]
            # 长传附件
            att = MIMEText(open(NewReport,'rb').read(),'base64','utf-8')
            att['content-Type'] = 'application/octet-stream'
            # 邮件类型附件，附件名称
            att.add_header('Content-Disposition','attachment',filename=('gbk','',"超级售后PC端自动化测试报告.html"))  # 文件名字有汉字设置编码
...
if __name__ == '__main__':
    ...
    Send_Email().SendEmailMain(start_time="10",run_time="20")