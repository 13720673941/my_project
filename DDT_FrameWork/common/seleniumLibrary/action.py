...
class Action(Find):
    ...
    @error_collection
    def drag_and_drop(self, firstElement, secondElement):
        """
        长按拖拽按钮从某元素到另一个元素
    ...
    @error_collection
    def click_hold_on(self, elementPath, isRelease=False, holdTime=3):
        """
        点击并长按按钮
    ...
    @error_collection
    def drag_and_drop_by_offset(self, elementPath, x_offset, y_offset, isRelease=False, holdTime=3):
        """
        长按按钮并移动到规定坐标量位置
        :param holdTime: 按钮长安时间默认 3s
        :param isRelease: 是否释放长安按钮
